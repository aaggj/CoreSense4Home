...
#include "arm/manipulation/point_at_tolerance.hpp"
#include <math.h> 
...
PointAtTolerance::PointAtTolerance(
...
void PointAtTolerance::on_tick()
{
  RCLCPP_DEBUG(node_->get_logger(), "PointAtTolerance ticked");
  ...
  if (tf_frame_.empty() && pose_to_point_) {
    ...
    goal_.pose.pose.position.x =  (std::isnan(x_point) || std::isinf(x_point)) ? 0.0 : (x_point) + tolerance_; 
...
BT::NodeStatus PointAtTolerance::on_success() {return BT::NodeStatus::SUCCESS;}
